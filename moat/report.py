...
class ReportGenerator:
    ...
    def _generate_pdf(self, output_path: str = "") -> str:
        """生成 PDF 合规报告"""
        if not output_path:
            output_path = str(self.project / "moat-report.pdf")
        ...
        try:
            ...
            pdf.cell(0, 15, "Moat Architecture Report", align="C", new_x="LMARGIN", new_y="NEXT")
            pdf.ln(5)

            # 元信息
            pdf.set_font("Helvetica", "", 10)
            pdf.cell(0, 6, f"Project: {self.project}", new_x="LMARGIN", new_y="NEXT")
            pdf.cell(0, 6, f"Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", new_x="LMARGIN", new_y="NEXT")
            pdf.cell(0, 6, f"Status: {'PASSED' if self.result.is_success() else 'FAILED'}", new_x="LMARGIN", new_y="NEXT")
            pdf.ln(5)
            ...
            pdf.cell(0, 10, "Check Summary", new_x="LMARGIN", new_y="NEXT")
            pdf.set_font("Helvetica", "", 10)
            summary = self.result.summary() if hasattr(self.result, 'summary') else str(self.result)
            ...
            if self.result.errors:
                pdf.set_font("Helvetica", "B", 14)
                pdf.cell(0, 10, f"Issues Found ({len(self.result.errors)})", new_x="LMARGIN", new_y="NEXT")
                pdf.set_font("Helvetica", "", 9)

                for idx, error in enumerate(self.result.errors[:50], 1):
                    file_info = error.get("file", "?")
                    message = error.get("message", "?")
                    level = error.get("level", "ERROR")

                    # 颜色: CRITICAL=红, WARNING=橙, 其他=黑
                    if level == "CRITICAL":
                    ...
                    pdf.multi_cell(0, 5, f"  {idx}. [{level}] {file_info}")
            ...
            pdf.cell(0, 5, f"Generated by Moat v1.1.6 | moat-ai on PyPI", align="C", new_x="LMARGIN", new_y="NEXT")
            ...
            print("   pip install 'moat-ai[pdf]' 安装 PDF 支持")
...
def generate_report(project_root: str = ".",
                    result: "MoatResult | None" = None,
                    format: str = "text", copy: bool = False,
                    output_path: str = "") -> str:
    """生成报告（CLI 入口）

    Args:
        project_root: 项目根目录
        result: MoatResult 对象
        format: 输出格式（text / md / json / pdf）
        copy: 是否复制到剪贴板
    ...
    if format == "pdf":
        return generator._generate_pdf(output_path)
    elif format == "json":
